...
const createProblem = async (req, res) => {
    ...
    const { 
        ...
        examples, constraints, testCases, referenceSolution 
    } = req.body;
    ...
    try {
        ...
        // (e.g., If admin provides Python & C++ solutions, check both)
        for (const [language, sourceCode] of Object.entries(referenceSolution)) {
            
            const langId = getLanguageId(language);
            if (!langId) throw new Error(`Unsupported language: ${language}`);

            // --- STEP A: PREPARE BATCH ---
            // Create a "job" for every single test case
            const batchSubmissions = testCases.map((tc) => ({
                ...
                // Judge0 will compare 'stdout' vs 'expected_output' for us!
            }));
            ...
            // We get an array of tokens: [{token: "abc"}, {token: "xyz"}]
            // Extract them into a comma-separated string for the GET request
            ...
            while (isProcessing && attempts < MAX_ATTEMPTS) {
                ...
                // Check if ANY submission is still In Queue (1) or Processing (2)
                const stillWorking = data.some(s => s.status.id === 1 || s.status.id === 2);

                if (!stillWorking) {
                    isProcessing = false;
                    results = data; // All done!
                } else {
            ...
            if (isProcessing) {
                return res.status(504).json({ error: "Judge0 timed out. Try again." });
            }
            // --- STEP D: ANALYZE RESULTS ---
            // Status ID 3 = "Accepted" (Passed)
            // Any other ID (4, 5, 6, etc.) is a failure.
            const failedCase = results.find(s => s.status.id !== 3);

            if (failedCase) {
                // Decode the error message to show the admin what went wrong
                ...
                return res.status(400).json({
                    ...
                    details: {
                        status: failedCase.status.description, // e.g., "Wrong Answer"
                        error_log: errorMsg
                    }
                });
            }
        ...
        // --- STEP E: SAVE TO DB ---
        // Only reachable if NO error was thrown above
        ...
        const newProblem = await prisma.problem.create({
            data: {
                ...
                examples,
                constraints,
                testCases, // Encrypted/Hidden in DB
        ...
        res.status(201).json({ message: "Problem Verified & Created", problem: newProblem });
...
const  getAllProblems =async(req , res) => {
    try{
        ...
        return res.status(500).json({
            error : "Error while feching problems",
        });
    }
};
const getProblemById =async(req , res) => { 
    ...
    try{
        ...
        console.log(error);
        return res.status(500).json({
            error :  "Error while feching Problem by id",
        });
    }
};
const updateProblemById =async(req , res) => {
    const {id} = req.params;
// we have to update the problem by id
...
const getSolvedProblemsByUser = async(req , res) => {
        try{
            ...
            console.log("Error while feching problems",error);
...
export {createProblem , getAllProblems , getProblemById , updateProblemById , deleteProblemById , getSolvedProblemsByUser};